alter table public.games
  add column if not exists assigned_externally boolean not null default false;

comment on column public.games.assigned_externally is
  'True when responsibility for assigning this game is handled outside Law18Ref.';
...
begin
  ...
  set starts_at = (requested_game ->> 'starts_at')::timestamptz,
      ...
      assigned_externally = coalesce((requested_game ->> 'assigned_externally')::boolean, target_game.assigned_externally, false),
...
begin
  ...
  update public.games set
    ...
    operational = second_game.operational, assigned_externally = second_game.assigned_externally,
    tags = second_game.tags,
  ...
  update public.games set
    ...
    operational = first_game.operational, assigned_externally = first_game.assigned_externally,
    tags = first_game.tags,
    schedule_changed_at = now(), schedule_changed_by = (select auth.uid()),
    schedule_change_summary = 'Game details swapped between schedule slots'
